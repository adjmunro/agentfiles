...
Created `skills/ideation/commands/ideate.md` — the thin orchestrator for the 9-step ideation flow.

### What was done

- Read reference files: `skills/kanban/commands/next.md` (orchestrator pattern), `skills/ideation/SKILL.md` (full 9-step flow), `skills/ideation/commands/capture.md` (phase file input contract).
- Wrote `skills/ideation/commands/ideate.md` with:
  - Valid YAML frontmatter (`model`, `allowed-tools`, `argument-hint`)
  - `## Personas` section (orchestrator voice only; delegates persona loading to phase files)
  - `## DO / DO NOT` rules (drive loop, delegate all phase work, no direct implementation)
  - 8 phases covering the full 9-step flow: subject setup, capture, research, interview, plan+audit, validate (step 6 user branch), tickets+audit, hard stop gate (step 9)
  - Subject slug resolution from `$ARGUMENTS` with `new`/empty fallback via `AskUserQuestion`
  - Loop-back logic at Phase 6: re-invokes from Phase 2 on "Add more"; `capture.md` handles append-only semantics
  - Abandon guard at Phase 6 and Phase 8: exact subject slug required, yes/no explicitly rejected
  - Step 9 hard stop: promotes `03-refinement/` → `04-todo/` or deletes entire subject directory
  - `from-ideation-handoff` token passed to kanban at step 9 promotion
