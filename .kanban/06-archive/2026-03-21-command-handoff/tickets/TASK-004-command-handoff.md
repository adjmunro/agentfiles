---
id: "2026-03-21-command-handoff/TASK-004"
...
plan_items:
  ...
  - "Req 4.4 — on confirmed: remove all files under .kanban/01-plan/YYYY-MM-DD-<subject>/"
acceptance_criteria:
  ...
  - "The Discard instructions in plan.md specify: (1) scan 02-06 for matching tickets, (2) block with description if found, (3) require typing exact subject slug if clear, (4) delete .kanban/01-plan/YYYY-MM-DD-<subject>/ on confirmed — all four steps verifiable by reading the file"
...
4. On confirmed: remove `.kanban/01-plan/YYYY-MM-DD-<subject>/` and all contents
...
- Step 3 (confirm): instructs agent to require user to type the exact subject slug (e.g. `2026-03-21-command-handoff`); states that yes/no is insufficient
- Step 4 (delete): instructs agent to remove all files under `.kanban/01-plan/YYYY-MM-DD-<subject>/`
...
- Step 4: On confirmed slug match, remove all files under `.kanban/01-plan/YYYY-MM-DD-<subject>/`
...
| All four steps verifiable in plan.md | Steps 1–4 at plan.md:243–269 cover all sub-requirements in order: scan 02-06, block+describe+stop, exact slug required, delete 01-plan/YYYY-MM-DD-subject/ | Satisfied |
