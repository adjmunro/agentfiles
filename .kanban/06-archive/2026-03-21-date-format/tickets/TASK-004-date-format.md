---
id: "2026-03-21-date-format/TASK-004"
...
acceptance_criteria:
  - "grep -r 'YYYY-MM-DD' skills/kanban/commands/ returns no matches"
...
Update all 9 command files under `skills/kanban/commands/` to replace `YYYY-MM-DD` with `YYYY-MM-DD` throughout — in phase instructions, bash command examples, commit message templates, and subject derivation steps. Also fix the `date` command template from `date +%y%m%d` to `date +%Y-%m-%d` in the three files that use it (capture.md, plan.md, init.md).
...
All 9 command files under `skills/kanban/commands/` updated to replace `YYYY-MM-DD` with `YYYY-MM-DD` throughout.

**Changes made:**

- `capture.md` — argument-hint, `$ARGUMENTS` match pattern, date prefix text + `date +%y%m%d` → `date +%Y-%m-%d`, directory path templates, file content template (`# YYYY-MM-DD-<subject>` and `## Session YYYY-MM-DD-HH:MM`), commit message template. 9 occurrences.
...
- `cleanup.md` — all 23 occurrences replaced via `replace_all` on the `YYYY-MM-DD-<subject>` pattern, plus the `[YYYY-MM-DD-subject]` argument description.
- `next.md` — argument-hint, diagram path label, subject format description, 10 occurrences of `YYYY-MM-DD-<subject>` in body prose, escalation block, progress summary.
...
**Verification:** `grep -r 'YYYY-MM-DD\|date +%y%m%d' skills/kanban/commands/` returns no matches.
...
| AC | Result | Evidence |
|---|---|---|
| 1. `grep -r 'YYYY-MM-DD' skills/kanban/commands/` returns no matches | PASS | No matches found |
