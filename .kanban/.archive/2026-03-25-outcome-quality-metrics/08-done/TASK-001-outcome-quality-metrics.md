...
completed_at: "2026-03-24T11:38:30Z"
stale_after_hours: 4
depends_on: []
spawned_tickets: []
plan_items:
...
| AC | Evidence | Status |
|----|----------|--------|
| Grep `MX-OQ` in `p2-baseline.md` returns no matches | Grep ran — 0 matches | Satisfied |
| Grep `Interview Signals` in `interview.md` returns no matches | Grep ran — 0 matches | Satisfied |
| Grep `00-quality` in `interview.md` returns no matches | Grep ran — 0 matches | Satisfied |
| Grep `how did this session go` in `p8-move-to-review.md` returns no matches | Grep ran — 0 matches | Satisfied |
| Grep `PR Responses` in `p5-fail.md` returns no matches | Grep ran — 0 matches | Satisfied |
| Grep `Plan Drift` in `cleanup.md` returns no matches | Grep ran — 0 matches | Satisfied |
| No `.kanban/*/00-quality-*.md` files exist | Glob ran — 0 files | Satisfied |
