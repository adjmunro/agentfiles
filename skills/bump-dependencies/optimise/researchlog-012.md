<!-- SUMMARY-START -->
## Run 012 — 2026-04-14 | Target: skills/bump-dependencies/
Composite: 86.2% → 90.2% (+4.0 pp)

### Hypotheses
| ID  | Description                                          | Outcome   |
|-----|------------------------------------------------------|-----------|
| H41 | Fix stale orchestrator comment headers in p1–p6     | Confirmed |
| H42 | Add composite-build glob to Step C                  | Confirmed |
| H43 | Add skip-category summary to Phase 0 PR body        | Confirmed |
| H44 | Add Gradle wrapper example to Step G                | Confirmed |

### Metric Snapshot
| Metric                                    | Baseline | Post |
|-------------------------------------------|----------|------|
| Version Catalogue Discovery Completeness  | 0        | 100  |
| Phase 0 PR Body Skip Coverage             | 0        | 100  |
| Stale Orchestrator Reference Rate         | 30       | 100  |
| Step G Commit Message Format Parity       | 67       | 100  |
| Pattern Experimental Validation Rate      | 20       | 40   |
| Composite                                 | 86.2%    | 90.2% |
<!-- SUMMARY-END -->
...
Tier C — target matches, log is from today (≤7 days). Run 9 Phase 5 written retroactively; proceeding to Run 10.
...
3. **Stale orchestrator comment headers identified:** p1-parse.md, p1b-split-commits.md, p2-investigate.md, p3-impact.md, p4-remediate.md, p5-verdict.md, p6-comment.md still reference `review-dependency-update.md` — the pre-v4.0.0 orchestrator filename. 7 of 10 phase-file comment headers are stale.
...
Applicable patterns for bump-dependencies: P3, P5, P7, P10, P11, P12, P13, P14, P16, P17 = 10 total.
...
**Measures:** Whether `<!-- Part of: ... orchestrator -->` comment headers in phase files accurately name the current orchestrator file.
...
**Methodology:** Count phase files with this comment header (p0–p8 = 10). Count accurate references (pointing to `bump-dependencies.md`).
...
**Measures:** Whether the PR body template in Phase 0 Step H includes a summary of dependencies that were considered but not bumped. A reviewer approving the PR cannot see what was skipped — only what was bumped.
**Why seeds miss it:** MX6 (Comment Template Completeness) measures Phase 6's per-bump review comments; it doesn't check Phase 0's PR creation template.
**Methodology:** Inspect Step H's PR body template. Check whether any skip-category information from Step I is present. Raw: 0/1 — PR body has only the bumps table; no skip summary. **Normalised: 0.**
**Direction:** ↑ higher is better
**Weight:** 1× — reviewers benefit from knowing what was evaluated
**Normalisation:** present / absent × 100

### MX38 — Changelog URL Annotation Freshness Risk (CUAFR) [custom, moonshot]
**Measures:** Whether Phase 0 Step E validates that the changelog URL it annotates into version files actually resolves.
**Methodology:** Inspect Step E. Check whether any instruction validates URL resolution before annotation. Raw: 0/1. **Normalised: 0.**
**Direction:** ↑ higher is better
**Weight:** 1× — practical impact low (changelog URLs for major libraries are stable)
**Normalisation:** present / absent × 100

### MX39 — Version Catalogue Discovery Completeness (VCDC) [custom]
**Measures:** Whether Step C's `libs.versions.toml` discovery covers non-standard paths used in Gradle composite builds (e.g., `build-logic/`, `buildSrc/`).
**Why seeds miss it:** MX29 measures Phase 2's code-usage search coverage; it doesn't measure Phase 0's discovery coverage.
**Methodology:** Inspect Step C. Check whether the discovery rules extend beyond the two hardcoded paths. Raw: 0/1 — only two specific paths listed; no catch-all glob for non-standard locations. **Normalised: 0.**
**Direction:** ↑ higher is better
**Weight:** 1× — projects with composite builds may have undetected outdated dependencies
**Normalisation:** present / absent × 100

### Inherited Metrics
All 49 metrics carry forward at Run 9 post-experiment values (6,679/7,300 = 91.5%). No instruction file changes since Run 9 affect any inherited metric.
...
**Risk level:** low
**Risk note:** Additive — existing paths are still checked first; catch-all only adds extra sources.

### H43 — Add skip-category summary to Phase 0 PR body
**Problem observed:** MX37 = 0. Phase 0 Step H's PR body template includes only the "Bumps included" table. A reviewer cannot see which dependencies were evaluated but skipped.
...
**Problem observed:** MX36 = 67. Step G provides commit message examples for libs.versions.toml and GitHub Actions, but not for the Gradle wrapper.
...
3. **Gap fill:** Remaining sub-100 after hypotheses: MX38=0 (moonshot), MX34=0 (moonshot), PEV=40 (structural), HTC=95 (deliberate), CLE=94 (structural), RI=98 (deliberate). No sub-100 metric below 33 remains with an actionable fix after H41. ✓
...
Step 0 — Pre-Experiment Dependency Scan: H41 has no file overlap with H42–H44. H42, H43, H44 all modify p0-bump.md (different sections) — run sequentially.
Execution order: H41 → H42 → H43 → H44.
...
**Result:** confirmed
**Notes:** HTML comment metadata only; no logic changed. P12 (Content Synchronisation Audit) validated.

### H42 — Add composite-build glob to Step C
**Pre-change (re-checked after H41):** MX39 = 0
**Post-change:** Catch-all glob added after hardcoded paths table.
- MX39: 100 (+100pp)
- PEV: P3 newly validated → 30 → 40 (+10pp)
**Delta:** MX39 +100pp, PEV +10pp
**Result:** confirmed
**Notes:** P3 (Progressive Disclosure): specific paths first, then catch-all. Additive-only.

### H43 — Add skip-category summary to Phase 0 PR body
**Pre-change (re-checked after H42):** MX37 = 0
**Post-change:** Dependency scan summary table added to Step H's PR body template.
- MX37: 100 (+100pp)
**Delta:** MX37 +100pp
**Result:** confirmed
**Notes:** Additive — new section inserted between "Bumps included" table and "Safety note". P10 (Failure Mode Registry) applied.

### H44 — Add Gradle wrapper example to Step G
**Pre-change (re-checked after H43):** MX36 = 67
**Post-change:** Wrapper-specific example added after GitHub Actions example. MX36: 100 (+33pp).
**Delta:** MX36 +33pp
**Result:** confirmed
**Notes:** All three ecosystems now have explicit commit message examples.
...
- Pattern Experimental Validation Rate (+20pp): P12 (Content Synchronisation Audit) and P3 (Progressive Disclosure) newly validated via H41 and H42.

### What was dropped
Nothing — all four hypotheses confirmed.

### What remains to improve
- Changelog URL Annotation Freshness Risk (MX38): 0 — moonshot; adding per-annotation HTTP validation would introduce N extra round-trips during Phase 0.
- Temporal Safety Window Consistency (MX34): 0 — moonshot; TOCTOU risk from static BUMP_DATE.
- Pattern Experimental Validation Rate (PEV): 40 — structural; 6 applicable patterns (P5, P11, P13, P14, P16, P17) not yet targeted in isolated confirmed hypotheses.
- Human Touchpoint Count (HTC): 95 — deliberate.
- Context Loading Efficiency (CLE): 94 — structural.

### Novel Patterns Observed
None this run. H41–H44 applied existing patterns (P12, P3, P10, P7).

Note from Run 9: H40 introduced a novel Cross-File Structural Anchor pattern. Still pending promotion to `skills/optimise/commands/phases/p3-hypothesize.md`.
