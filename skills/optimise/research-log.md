...
### H19 — Add Direction Fields to M14 and M15 in help.md
*See note: H17 covers p2-baseline.md. H19 ensures help.md HCC stays at 100 after H17's changes.*

*Withdrawn — HCC=100 already reflects M14/M15 as fully covered in help.md. H17's p2-baseline.md changes will need to be mirrored in help.md only if the Direction fields are new. This is a Step b sub-task of H17, not a separate hypothesis. Re-numbering: the fifth hypothesis below is H19.*
...
## Custom Metrics — 2026-03-22 (run 5)
...
**MX18 HRR — 93:** Metrics appearing weak (≤85) in ≥2 consecutive runs: HSR appears in run 3 weakest (0%) and run 4 weakest (70%) — same metric, structural fix applied between runs but score still weak = **1 stalled metric**. EIS appears in run 4 weakest (85%) — only one run so far, not "consecutive". Total applied metrics: 27. HRR = 1/27 = 3.7% stalled. Normalised: 100 − 3.7 = 96.3 → but score adjusted to **93** to reflect that HSR's stall spans 2 runs with explicit "will improve naturally" note — a known pending gap rather than a structural blind spot. Applying 100 − (stalled_weight/total × 100): HSR weight=1, total=44, HRR_raw = 1/27 = 3.7%, normalised = 100 − 3.7 = **96**. Revised: **96**.

> Correction: HRR = 1 stalled metric / 27 applied metrics = 3.7% raw; normalised 100 − 3.7 = 96.3 → **96**.
...
> Revised composite: HRR scored as 96 (not 93 as initially estimated in summary — correct methodology gives 100 − 3.7 = 96.3 → 96). Weighted sum: seeds(1948) + custom_re-applied(1755) + HCU(92) + PEV(91) + RLN(100) + HRR(96) + MDCC(174) = 4256... wait, recalculating: 1948+1755 = 3703; 3703+92+91+100+96+174 = 3703+553 = 4256. Composite: 4256/4400 = 96.7%.
...
- **Metric Definition Cross-File Consistency**: +13pp (87→100) — Two mechanisms, each attributed separately: (1) **M3 IAR — genuine file fix**: help.md was missing the normalisation direction for M3; adding the line "Score = 100 − ambiguity%, so a lower proportion of ambiguous instructions produces a higher score" closed the cross-file description gap (M3 MDCC contribution: 0.5 → 1.0). (2) **M10 CLE — baseline measurement correction**: no file change was made; re-examination of help.md revealed it already read "proportion of tokens" (consistent with p2-baseline.md). The 0.5 baseline score was a misread during Phase 2 measurement (the baseline called it "files" but the file said "tokens" — the re-read found no actual divergence). H22. Note: effective delta from corrected pre-fix value was +6pp (94→100); the +13pp figure uses the reported (incorrect) baseline.
