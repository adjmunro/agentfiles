...
## v2.0.0 - 2026-03-22 - Subject-Centric Restructure
...
- Subject directories now use ISO date format: `YYYY-MM-DD-{subject}/`
- Stage directories renumbered: `04-todo/` → `05-in-progress/` → `06-in-review/` → `07-pull-request/` → `08-done/`
- Kanban2 skips all plan-layer files: never reads or writes `00-input-`, `01-research-`, `02-plan-`, `00-assets/`, or `03-refinement/`
- Work loop picks up tickets from `04-todo/`, routes through review/PR/cleanup, and finishes in `08-done/`
- Thin orchestrator pattern: next.md-equivalent driver for work→review escalation, stale detection, and consecutive-failure logic
- Persona identification retained: commands identify active persona (Vela, Arden, Finn, Kira, Echo, Vale) in all transcripts
- Shared persona files: references `../../kanban/personas/` rather than duplicating
